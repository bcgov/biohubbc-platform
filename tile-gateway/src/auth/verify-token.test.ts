...
import { verifyTileToken } from './verify-token.js';
...
describe('verifyTileToken', () => {
  describe('valid tokens', () => {
    it('accepts a correctly signed token and returns its claims', () => {
      const claims = verifyTileToken(bearer({ ctx: 'ctx-abc', jti: 'jti-abc' }), TEST_SOURCE);
    ...
    it('accepts a token signed with the second published key (rotation window)', () => {
      ...
      expect(verifyTileToken(token, TEST_SOURCE).source).to.equal(TEST_SOURCE);
    });
  });

  describe('401 - the caller should re-mint', () => {
    it('rejects a missing Authorization header', () => {
      expectStatus(() => verifyTileToken(undefined, TEST_SOURCE), 401);
    });

    it('rejects a non-bearer Authorization header', () => {
      expectStatus(() => verifyTileToken('Basic abc123', TEST_SOURCE), 401);
    });

    it('rejects a malformed token', () => {
      expectStatus(() => verifyTileToken('Bearer not-a-jwt', TEST_SOURCE), 401);
    });
    ...
    it('rejects an invalid signature (token signed by an unpublished key)', () => {
      ...
      expectStatus(() => verifyTileToken(`Bearer ${token}`, TEST_SOURCE), 401);
    });
    ...
    it('rejects an expired token', () => {
      expectStatus(() => verifyTileToken(bearer({ expiresIn: -60 }), TEST_SOURCE), 401);
    });
    ...
    it('rejects an unsigned "alg: none" token', () => {
      ...
      expectStatus(() => verifyTileToken(`Bearer ${header}.${payload}.`, TEST_SOURCE), 401);
    });
    ...
    it('rejects a token whose payload was edited after signing', () => {
      ...
      expectStatus(() => verifyTileToken(`Bearer ${header}.${forgedPayload}.${signature}`, TEST_SOURCE), 401);
    });

    it('rejects an incorrect audience', () => {
      expectStatus(() => verifyTileToken(bearer({ audience: 'some-other-audience' }), TEST_SOURCE), 401);
    });

    it('rejects an incorrect issuer', () => {
      expectStatus(() => verifyTileToken(bearer({ issuer: 'evil-issuer' }), TEST_SOURCE), 401);
    });

    it('rejects a token missing the context claim', () => {
      expectStatus(() => verifyTileToken(bearer({ omit: ['ctx'] }), TEST_SOURCE), 401);
    });

    it('rejects a token missing the jti claim', () => {
      expectStatus(() => verifyTileToken(bearer({ omit: ['jti'] }), TEST_SOURCE), 401);
    });
    ...
    it('does not disclose why verification failed', () => {
      ...
      const messageFor = (authorization: string): string => {
        try {
          verifyTileToken(authorization, TEST_SOURCE);
          expect.fail('expected verification to throw');
        } catch (error) {
  ...
  describe('403 - the token will never work for this request', () => {
    it('rejects a token scoped to a different source', () => {
      expectStatus(() => verifyTileToken(bearer({ source: 'some-other-source' }), TEST_SOURCE), 403);
    });

    it('rejects a token without the required scope', () => {
      expectStatus(() => verifyTileToken(bearer({ scope: 'something:else' }), TEST_SOURCE), 403);
    });

    it('rejects a token with no scope claim', () => {
      expectStatus(() => verifyTileToken(bearer({ omit: ['scope'] }), TEST_SOURCE), 403);
    });
  });
});
