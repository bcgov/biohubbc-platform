...
import { verifyTileToken } from '../auth/verify-token.js';
...
const defaultLog = getLogger('routes/tiles');

/**
 * The ONLY request shape the gateway serves: `/tiles/{source}/{z}/{x}/{y}`.
 *
 * The source segment is restricted to word characters, so a composite source request such as
 * `/tiles/a,b/1/0/0` cannot match. Martin supports composite sources, and they would let a caller
 ...
 */
export const TILE_PATH_REGEX = /^\/tiles\/([A-Za-z0-9_]+)\/(\d{1,2})\/(\d{1,10})\/(\d{1,10})$/;
...
export const parseTilePath = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const match = TILE_PATH_REGEX.exec(req.path);
...
export const authenticateTileRequest = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.locals.tokenClaims = verifyTileToken(req.headers.authorization, source);
