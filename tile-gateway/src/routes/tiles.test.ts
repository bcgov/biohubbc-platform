...
describe('tile route', () => {
  ...
  describe('allowlisting', () => {
    it('serves the one allowlisted tile path', async () => {
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization: bearer() });
    ...
    it('rejects the catalog even under the /tiles prefix', async () => {
      const response = await rawGet(server, '/tiles/catalog', { authorization: bearer() });
    ...
    it('rejects a TileJSON request', async () => {
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}`, { authorization: bearer() });
    ...
    it('rejects a composite source request', async () => {
      ...
      const response = await rawGet(server, `/tiles/${TEST_SOURCE},secret_source/5/5/11`, {
    ...
    it('rejects an unapproved source', async () => {
      const response = await rawGet(server, '/tiles/some_other_source/5/5/11', {
    ...
    it('rejects a path traversal attempt', async () => {
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/../../../catalog`, {
        authorization: bearer()
      });
    ...
    it('rejects a zoom level above the configured maximum', async () => {
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}/20/5/11`, { authorization: bearer() });
    ...
    it('rejects tile coordinates that cannot exist at the requested zoom', async () => {
      ...
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}/2/99/1`, { authorization: bearer() });
  ...
  describe('authorization', () => {
    it('rejects a request with no token before contacting Martin', async () => {
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`);
  ...
  describe('request sanitisation', () => {
    it('strips every client supplied query parameter and injects only the trusted context', async () => {
      await rawGet(
        server,
        `/tiles/${TEST_SOURCE}/5/5/11?context=attacker-context&filter=1%3D1&ctx=cache-buster&user_id=42`,
    ...
    it('never forwards the client Authorization header to Martin', async () => {
      await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization: bearer() });
  ...
  describe('response metadata', () => {
    it('preserves content type, ETag, cache headers and gzip encoding without recompressing', async () => {
      const response = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization: bearer() });
    ...
    it('passes an empty tile through as a cacheable 204', async () => {
      ...
      const second = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization: bearer() });
  ...
  describe('caching', () => {
    it('serves a repeat request from cache', async () => {
      ...
      const first = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization });
      const second = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization });

      expect(first.headers['x-tile-cache']).to.equal('MISS');
      expect(second.headers['x-tile-cache']).to.equal('HIT');
      expect(martin.requests).to.have.length(1);
    });

    it('never shares cached tiles between different authorization contexts', async () => {
      await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization: bearer({ ctx: 'ctx-alice' }) });
      const bob = await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization: bearer({ ctx: 'ctx-bob' }) });

      // Bob's request must reach Martin rather than reuse Alice's tile.
      expect(bob.headers['x-tile-cache']).to.equal('MISS');
    ...
    it('deduplicates concurrent requests for the same tile', async () => {
      ...
      const responses = await Promise.all(
        Array.from({ length: 5 }, () => rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization }))
      );
  ...
  describe('logging', () => {
    it('never writes the Authorization header or raw token to the logs', async () => {
      ...
      try {
        await rawGet(server, `/tiles/${TEST_SOURCE}/5/5/11`, { authorization });
      } finally {
