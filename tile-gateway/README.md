...
```
MapLibre -> tile gateway (this service) -> Martin -> PostGIS
```
...
| Route | Purpose |
| --- | --- |
| `GET /tiles/{source}/{z}/{x}/{y}` | The only tile route. Requires `Authorization: Bearer <tile token>`. |
...
| --- | --- | --- |
| `TILE_GATEWAY_PORT` | `6300` | Listen port. |
| `MARTIN_URL` | `http://127.0.0.1:3000` | Upstream Martin. Loopback in OpenShift (sidecar). |
| `MARTIN_TIMEOUT_MS` | `10000` | Upstream timeout. |
| `TILE_TOKEN_PUBLIC_KEY_DIR` | *(required)* | Directory of `<kid>.pem` public keys. |
| `TILE_TOKEN_AUD` | `biohub-tiles` | Expected `aud` claim. |
| `TILE_TOKEN_ISS` | `biohub-api` | Expected `iss` claim. |
| `TILE_TOKEN_SCOPE` | `tiles:read` | Scope the token must carry. |
| `TILE_ALLOWED_SOURCES` | `fixture` | Comma separated sources this gateway will serve. |
| `TILE_MIN_ZOOM` / `TILE_MAX_ZOOM` | `0` / `15` | Inclusive zoom bounds. |
| `TILE_CACHE_TTL_SECONDS` | `300` | Cache lifetime. |
| `TILE_CACHE_MAX_BYTES` | `52428800` | Cache size cap, by total body bytes. |
| `TILE_SOURCE_VERSION` | `1` | Mixed into every cache key. Bump to invalidate all cached tiles. |
...
Keys are loaded from `TILE_TOKEN_PUBLIC_KEY_DIR`, one file per key id (`<kid>.pem`). Holding several
...
# Tile gateway only - the verification key(s), named by kid
oc create secret generic tile-token-public --from-file=2026-07.pem -n <namespace>
```

Locally the keypair is generated automatically into a docker volume by the `tile_keys_setup` compose
service; no manual step is needed.

## Local development

```bash
make tiles        # start the signing keys, Martin, and this gateway
make log-tile-gateway
```
...
```bash
# Mint a session
curl -s -X POST http://localhost:6200/api/tile/token

# Fetch a tile
curl -s -H "Authorization: Bearer <token>" http://localhost:6300/tiles/fixture/5/5/11
```
...
```bash
npm test                 # unit suite
make test-tiles          # integration suite, against the running compose stack
```
