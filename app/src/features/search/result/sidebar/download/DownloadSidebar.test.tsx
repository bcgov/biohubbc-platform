...
describe('DownloadSidebar', () => {
  ...
  it('renders checkout button with correct label', () => {
    const { getByRole } = render(<DownloadSidebar {...defaultProps} cart={{ features: mockFeatures, itemCount: 2 }} />);
  ...
  it('disables the checkout button when cart is empty', () => {
    const { getByRole } = render(<DownloadSidebar {...defaultProps} cart={{ features: [], itemCount: 0 }} />);
  ...
  it('calls onDownload when checkout button is clicked', () => {
    ...
    const { getByRole } = render(
      <DownloadSidebar {...defaultProps} cart={{ features: mockFeatures, itemCount: 2 }} onDownload={handleDownload} />
    );
