 ...
 *
 * @param {string} tileUrlTemplate - Template from the tile session, e.g. `/tiles/search/{z}/{x}/{y}`.
 * @param {string} contextId - Opaque tile context id, used only to vary the URL.
 * @return {*}  {SourceSpecification}
 */
export const buildSearchResultsSource = (tileUrlTemplate: string, contextId: string): SourceSpecification => {
  const absoluteTemplate = tileUrlTemplate.startsWith('http')
    ? tileUrlTemplate
    : `${window.location.origin}${tileUrlTemplate}`;
