...
describe('useSearchResultPagingSort', () => {
  it('optimistically updates the active sort before pagination refreshes', () => {
    ...
    expect(setSearchParams).toHaveBeenCalledWith({ sort: 'create_date', order: 'desc' });
  });
  ...
  it('uses the returned keyset cursor for the next page', () => {
    ...
    act(() => result.current.handlePageChange('next-token'));

    expect(setSearchParams).toHaveBeenCalledWith({ cursor: 'next-token' });
  });
});
