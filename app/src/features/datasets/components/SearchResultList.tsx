...
export type IDatasetVisibility = Record<string, boolean>
...
const SearchResultList: React.FC<ISearchResultListProps> = (props) => {
  ...
  const toggleVisibility = (key: string) => {
    ...
    const isVisible = !datasetVisibility[key]
    visibility[key] = !datasetVisibility[key] // isVisible
    */

    props.onChangeDatasetVisibility({ ...udpated });
  };
            ...
            return (
              <ListItem
                ...
                <ListItemButton onClick={() => toggleVisibility(item.key)} dense>
                  ...
                  <ListItemText primary={item.name} secondary={`${item.count} record${item.count !== 1 && 's'}`}></ListItemText>
                </ListItemButton>
              </ListItem>
            );
          })}
