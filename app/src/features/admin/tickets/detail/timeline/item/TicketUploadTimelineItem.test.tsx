...
const baseProps = {
  upload,
  dateLabel: 'Today',
  isCreatingReview: vi.fn().mockReturnValue(false),
...
describe('TicketUploadTimelineItem', () => {
  ...
  it('shows a spinner on only the Review action being created', () => {
    const uploadWithoutValidationReview = {
      ...upload,
      reviews: {
        ...upload.reviews,
    ...
    render(
      <TicketUploadTimelineItem
        {...baseProps}
        upload={uploadWithoutValidationReview}
        isCreatingReview={(scope) => scope === 'validation'}
      />
    );
