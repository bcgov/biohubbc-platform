...
describe('useTicketTimelineUploadActions', () => {
  ...
  it('creates an in-progress security review and navigates to the returned review', async () => {
    ...
    expect(result.current.creatingReviewKeys.has(`${submissionUploadId}:security`)).toBe(true);
    ...
    expect(result.current.creatingReviewKeys.has(`${submissionUploadId}:security`)).toBe(false);
  });
  ...
  it('reports a validation review failure without navigating and allows a retry', async () => {
    ...
    expect(result.current.creatingReviewKeys.has(`${submissionUploadId}:validation`)).toBe(false);
