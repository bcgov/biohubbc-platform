...
export const useTicketTimelineUploadActions = () => {
  ...
  const [creatingReviewKeys, setCreatingReviewKeys] = useState<Set<string>>(new Set());
  ...
  const handleCreateSubmissionUploadReview = async (
    ...
    const reviewKey = `${upload.submission_upload_id}:${scope}`;

    setCreatingReviewKeys((current) => new Set(current).add(reviewKey));
