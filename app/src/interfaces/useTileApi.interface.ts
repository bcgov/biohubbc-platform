/**
 * A tile session the map can render.
 */
export interface ITileSession {
   ...
   */
  tile_context_id: string;
  /** Tile URL template for MapLibre, e.g. `/tiles/search/{z}/{x}/{y}`. */
 ...
 */
export interface ITileSessionOverCap {
  over_cap: true;
  /** Maximum mappable features. */
  cap: number;
}

export type CreateTileSessionResponse = ITileSession | ITileSessionOverCap;
