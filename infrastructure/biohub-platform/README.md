...
- `database-setup` depends on `biohub-platform-db` and has a `post-install` hook with weight `1` to run after the database is deployed
- `api` depends on `biohub-platform-db-setup` and has a `post-install` hook with weight `2` to run after database-setup completes
