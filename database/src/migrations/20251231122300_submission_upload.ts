...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    CREATE TYPE upload_status AS ENUM (
      'pending',    -- Upload was requested
      'completed',  -- Upload session has finished
    ...
    CREATE INDEX upload_status_idx ON upload(upload_status);
    ...
    COMMENT ON COLUMN upload.upload_status IS 'Lifecycle state of the upload session.';
    ...
    CREATE TABLE artifact (
      ...
      CONSTRAINT check_status_when_uploaded_at_null
        CHECK (
          uploaded_at IS NOT NULL OR artifact_status = 'pending'
        )
    );
    ...
    COMMENT ON COLUMN artifact.artifact_status IS 'Indicates whether artifact has been uploaded or is in another state.';
    ...
    CREATE TABLE upload_archive (
      ...
      archive_status process_status NOT NULL,
      create_date timestamptz(6) DEFAULT now() NOT NULL,
      create_user integer NOT NULL,
      update_date timestamptz(6),
      update_user integer,
    ...
    COMMENT ON COLUMN upload_archive.archive_status IS 'Processing status of the archive, for extraction of contents.';
    ...
    CREATE UNIQUE INDEX security_uk ON artifact_security (artifact_id, security) WHERE record_end_date IS NULL;

    CREATE INDEX security_idx ON artifact_security(security);
    ...
    CREATE TABLE artifact_security_scan (
      ...
      scan_status process_status NOT NULL,
    ...
    CREATE INDEX scan_status_idx ON artifact_security_scan(scan_status);

    COMMENT ON TABLE artifact_security_scan IS 'Tracks individual malware scan events performed against a securityd artifact. Processing scan_status tracks lifecycle of scan execution (pending → completed/failed). For archive uploads, the scan is performed on the archive, and all extracted files reference this scan via their security record.';
    ...
    COMMENT ON COLUMN artifact_security_scan.scan_status IS 'Processing status of the scan execution.';
    ...
    CREATE TABLE artifact_security_scan_file (
      ...
      result security_status NOT NULL,
      create_date timestamptz(6) DEFAULT now() NOT NULL,
      create_user integer NOT NULL,
      update_date timestamptz(6),
      update_user integer,
    ...
    CREATE INDEX artifact_security_scan_file_result_idx ON artifact_security_scan_file(result);
    ...
    COMMENT ON COLUMN artifact_security_scan_file.result IS 'Security result of the malware scan for this file (pending, clean, infected, error, skipped).';
