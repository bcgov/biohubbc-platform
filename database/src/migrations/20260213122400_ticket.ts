...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    COMMENT ON TYPE decision_type IS 'Admin decision on an entity: APPROVED (accepted), DENIED (rejected), PENDING (awaiting decision).';
    ...
    --------------------------------------------------------------------------------
    -- DECISION (Central decision record)
    --------------------------------------------------------------------------------
    ...
    CREATE TABLE decision (
      ...
      update_user integer,
      revision_count integer DEFAULT 0 NOT NULL,
      CONSTRAINT decision_pk PRIMARY KEY (decision_id),
      CONSTRAINT decision_comment_fk FOREIGN KEY (comment_id) REFERENCES comment(comment_id)
    );

    CREATE INDEX decision_decision_idx ON decision(decision);
    CREATE INDEX decision_create_date_idx ON decision(create_date);

    COMMENT ON TABLE decision IS 'Central table for admin decisions. Each decision can be referenced by entity-specific decision tables. Entity status is derived from the most recent decision.';
    COMMENT ON COLUMN decision.decision_id IS 'System generated surrogate primary key identifier.';
    COMMENT ON COLUMN decision.decision IS 'The decision made: APPROVED, DENIED, or PENDING.';
    COMMENT ON COLUMN decision.comment_id IS 'Optional comment explaining the decision (e.g., reason for denial).';
    COMMENT ON COLUMN decision.record_end_date IS 'Timestamp for soft delete; null when record is active.';
    COMMENT ON COLUMN decision.create_date IS 'The datetime the decision was made.';
    COMMENT ON COLUMN decision.create_user IS 'The id of the user (admin) who made the decision.';
    COMMENT ON COLUMN decision.update_date IS 'The datetime the record was last updated.';
    COMMENT ON COLUMN decision.update_user IS 'The id of the user who last updated the record.';
    COMMENT ON COLUMN decision.revision_count IS 'Revision count used for concurrency control.';
    ...
    CREATE TABLE ticket_decision (
      ...
      CONSTRAINT ticket_decision_decision_fk FOREIGN KEY (decision_id) REFERENCES decision(decision_id)
    );
    ...
    COMMENT ON COLUMN ticket_decision.decision_id IS 'Foreign key to the decision record.';
    ...
    CREATE TABLE data_request_decision (
      ...
      CONSTRAINT data_request_decision_decision_fk FOREIGN KEY (decision_id) REFERENCES decision(decision_id)
    );
    ...
    COMMENT ON COLUMN data_request_decision.decision_id IS 'Foreign key to the decision record.';
    ...
    CREATE TABLE submission_upload_decision (
      ...
      CONSTRAINT submission_upload_decision_decision_fk FOREIGN KEY (decision_id) REFERENCES decision(decision_id)
    );
    ...
    COMMENT ON COLUMN submission_upload_decision.decision_id IS 'Foreign key to the decision record.';
    ...
    --   CONSTRAINT download_decision_decision_fk FOREIGN KEY (decision_id) REFERENCES decision(decision_id)
    -- );
    --
    -- CREATE INDEX download_decision_download_idx ON download_decision(download_id);
    -- CREATE INDEX download_decision_decision_idx ON download_decision(decision_id);
    ...
    -- COMMENT ON COLUMN download_decision.decision_id IS 'Foreign key to the decision record.';
    ...
    CREATE TRIGGER audit_decision
      BEFORE INSERT OR UPDATE OR DELETE ON decision
      FOR EACH ROW EXECUTE PROCEDURE biohub.tr_audit_trigger();

    CREATE TRIGGER journal_decision
...
export async function down(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    DROP TRIGGER IF EXISTS journal_decision ON decision;
    DROP TRIGGER IF EXISTS audit_decision ON decision;
