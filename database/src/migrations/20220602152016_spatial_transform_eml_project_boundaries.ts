...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    insert into spatial_transform (name, description, record_effective_date, transform) values ('EML Dataset Boundaries', 'Extracts EML dataset geographic coverage boundaries and properties.', now(), $transform$with submissions as (select eml_json_source from submission_metadata where submission_id = ?)
    ...
    , coverages as (select * from project_coverage union select * from related_project_coverages)
    , polys as (select c.cov_n, c.project_type, p.poly_n, p.points points from coverages c, jsonb_path_query(coverage, '$.**.datasetGPolygon[*].datasetGPolygonOuterGRing.gRingPoint') with ordinality p(points, poly_n))
    , latlongs as (select p.cov_n, p.project_type, p.poly_n, arr.point_n, arr.point->>'gRingLatitude' lat, arr.point->>'gRingLongitude' long from polys p, jsonb_array_elements(points) with ordinality arr(point, point_n))
