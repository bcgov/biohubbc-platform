 ...
 *
 * Generates a GeoJSON FeatureCollection for the boundary specified in the geographicCoverage section of an EML file.
 *
  ...
  const transformSQL = `
    ...
    project_coverage as (
      ...
      from
        ...
        jsonb_path_query(
          eml_json_source,
          '$.**.project.studyAreaDescription.**.geographicCoverage'
        ) with ordinality c(coverage, cov_n)
    ),
    related_project_coverages as (
      ...
      from
        ...
        jsonb_path_query(
          eml_json_source,
          '$.**.relatedProject.studyAreaDescription.**.geographicCoverage'
        ) with ordinality c(coverage, cov_n)
    ),
    descriptions as (
      select
    ...
    features as (
      ...
      from
        (
          ...
          where
            d.cov_n = m.cov_n
            and d.project_type = m.project_type
        ) f
    )
    select
      json_build_object(
