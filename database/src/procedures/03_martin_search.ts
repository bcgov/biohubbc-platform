...
export async function seed(knex: Knex): Promise<void> {
  await knex.raw(`--sql
    ...
    --
    -- The single place the security predicate is written. Both zoom branches of martin_search call
    ...
    AS $fn$
      ...
      WHERE g.value && p_envelope_4326
        ...
        AND (
          ...
          OR EXISTS (
            SELECT 1
            FROM biohub.martin_context_feature tcf
            WHERE tcf.martin_context_id = p_martin_context_id
    ...
    COMMENT ON FUNCTION biohub.martin_search_visible_geometries(uuid, boolean, integer, text, uuid[], public.geometry) IS
      'Resolves the geometries a tile context may see within a bounding box. Ported from isEffectivelySecured/isAccessibleToUser in api/src/repositories/sql-fragments.ts, with the team membership join replaced by the scope ids captured on the context. Called only from biohub.martin_search.';
    ...
    BEGIN
      ...
      REVOKE ALL ON FUNCTION biohub.martin_search_visible_geometries(uuid, boolean, integer, text, uuid[], public.geometry) FROM PUBLIC;
      ...
      IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = v_api_role) THEN
        ...
        EXECUTE format('GRANT EXECUTE ON FUNCTION biohub.martin_search_visible_geometries(uuid, boolean, integer, text, uuid[], public.geometry) TO %I', v_api_role);
      END IF;
    END
    $grants$;
  `);
