...
export async function seed(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    insert into feature_property_type (name, description, record_effective_date) values ('s3_key',   'An S3 key type (string)',    now()) ON CONFLICT DO NOTHING;
    ...
    insert into feature_property (name, display_name, description, feature_property_type_id, parent_feature_property_id, record_effective_date, calculated_value) values ('s3_key',              'Key',                  'The S3 storage key for an artifact',       (select feature_property_type_id from feature_property_type where name = 'string'),   null,                                                                         now(), true) ON CONFLICT DO NOTHING;
    insert into feature_property (name, display_name, description, feature_property_type_id, parent_feature_property_id, record_effective_date, calculated_value) values ('filename',            'Filename',             'The original name of an artifact, including extension', (select feature_property_type_id from feature_property_type where name = 'string'),   null,                                                                         now()) ON CONFLICT DO NOTHING;
    ...
    insert into feature_type_property (feature_type_id, feature_property_id, sort, record_effective_date) values ((select feature_type_id from feature_type where name = 'artifact'), (select feature_property_id from feature_property where name = 's3_key'),   2, now()) ON CONFLICT DO NOTHING;
