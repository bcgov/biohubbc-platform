...
describe('submission upload decision handler', () => {
  ...
  it('verifies the upload belongs to the submission, records the decision and commits', async () => {
    ...
    const ownershipStub = sinon.stub(SubmissionUploadService.prototype, 'getSubmissionUploadBySubmissionId').resolves();
    ...
    expect(ownershipStub).to.have.been.calledOnceWith(SUBMISSION_ID, SUBMISSION_UPLOAD_ID);
  ...
  it('rolls back and rethrows when the decision is refused', async () => {
    const connection = stubConnection();
    sinon.stub(SubmissionUploadService.prototype, 'getSubmissionUploadBySubmissionId').resolves();
