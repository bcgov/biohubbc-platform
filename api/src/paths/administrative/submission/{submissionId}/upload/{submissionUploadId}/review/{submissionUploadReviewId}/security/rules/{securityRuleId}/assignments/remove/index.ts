...
POST.apiDoc = {
  description:
    'Remove direct assignments for a rule within the reviewed upload. Without explicit IDs, the expression selects matches; omitting both selects the whole upload.',
  tags: ['admin'],
  security: [{ Bearer: [] }],
  parameters: [
...
/**
 * Handle POST for one rule's direct assignments; request scope is constrained to the reviewed upload.
 * @returns {RequestHandler} Transactional HTTP handler.
 */
export function deleteSubmissionUploadReviewSecurityRuleAssignments(): RequestHandler {
