...
export function getSubmissionUploadFeatureProperties(): RequestHandler {
  return async (req, res) => {
    ...
    try {
      ...
      await new SubmissionFeatureService(connection).getSubmissionUploadFeature(
        submissionId,
        req.params.submissionUploadId,
        submissionFeatureId
      );
      const result = await new SubmissionFeaturePropertyService(
        connection
      ).getSubmissionFeaturePropertiesBySubmissionUploadId(
        req.params.submissionUploadId,
        submissionFeatureId,
        pagination,
