...
describe('searchFeatures', () => {
  ...
  it('should return expression search results for secured features visible to the caller', async () => {
    ...
    expect(mockRes.statusValue).to.equal(200);
    // A visible secured row the caller CAN see must not, by itself, set has_inaccessible_secured_features.
    expect(mockRes.jsonValue).to.eql({
      features: mockResults,
      properties: [],
      has_inaccessible_secured_features: false,
  ...
  it('should expose has_inaccessible_secured_features when secured matches are hidden from the caller', async () => {
    const dbConnectionObj = getMockDBConnection({
      commit: sinon.stub().resolves(),
      rollback: sinon.stub().resolves(),
      release: sinon.stub().resolves(),
    ...
    expect(mockRes.statusValue).to.equal(200);
    expect(mockRes.jsonValue.has_inaccessible_secured_features).to.equal(true);
