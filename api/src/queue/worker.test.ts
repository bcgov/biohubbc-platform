...
import * as deleteExpiredTileContextsJob from './jobs/delete-expired-tile-contexts-job';
...
describe('worker', () => {
  ...
  describe('registerWorkers', () => {
    it('creates queues and registers handlers', async () => {
      ...
      expect(createQueueStub.calledWith(JobQueues.DELETE_EXPIRED_TILE_CONTEXTS)).to.be.true;
      ...
      expect(
        workStub.calledWith(
          ...
          deleteExpiredTileContextsJob.deleteExpiredTileContextsJobHandler
        )
      ).to.be.true;
      expect(
        workStub.calledWith(
          JobQueues.DELETE_EXPIRED_TILE_CONTEXTS_FAILED,
          deleteExpiredTileContextsJob.deleteExpiredTileContextsFailedHandler
        )
      ).to.be.true;
      expect(
        workStub.calledWith(
    ...
    it('schedules the poll on a recurring UTC cron after creating and working its queue', async () => {
      ...
      const tileSweepCall = scheduleStub
        .getCalls()
        .find((call) => call.args[0] === JobQueues.DELETE_EXPIRED_TILE_CONTEXTS);
