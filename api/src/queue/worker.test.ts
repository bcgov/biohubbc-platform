...
describe('worker', () => {
  ...
  describe('registerWorkers', () => {
    it('creates queues and registers handlers', async () => {
      ...
      expect(workStub.calledWith(JobQueues.PROCESS_SUBMISSION_FEATURES, processSubmissionFeaturesJob.processSubmissionFeaturesJobHandler)).to.be.true;
      expect(workStub.calledWith(JobQueues.MALWARE_SCAN, malwareScanJob.malwareScanJobHandler)).to.be.true;
      expect(workStub.calledWith(JobQueues.INDEX_SUBMISSION_FEATURES, indexSubmissionFeaturesJob.indexSubmissionFeaturesJobHandler)).to.be.true;
      expect(workStub.calledWith(JobQueues.COMPUTE_SCOPE_ANCHORS, computeScopeAnchorsJob.computeScopeAnchorsJobHandler)).to.be.true;
    });
    ...
    it('configures dead letter queue for process-submission-features', async () => {
      ...
      const processQueueCall = createQueueStub.getCalls().find((call) => call.args[0] === JobQueues.PROCESS_SUBMISSION_FEATURES);
