...
const defaultLog = getLogger('queue/jobs/delete-expired-martin-contexts-job');
 ...
 */
export type IDeleteExpiredMartinContextsJobData = Record<string, never>;

/**
 * Recurring sweep that removes expired map tile contexts and their materialized feature ids.
 *
 * Housekeeping only, never a security control: expiry is enforced in `biohub.martin_search` on every
 ...
 *
 * @param {PgBoss.Job<IDeleteExpiredMartinContextsJobData>[]} jobs - The sweep jobs to process.
 * @return {Promise<void>}
 */
export const deleteExpiredMartinContextsJobHandler: PgBoss.WorkHandler<IDeleteExpiredMartinContextsJobData> = async (
  jobs
) => {
  for (const job of jobs) {
    try {
      const deleted = await withConnection((connection) =>
        new MartinContextService(connection).deleteExpiredMartinContexts()
      );

      defaultLog.info({
        label: 'deleteExpiredMartinContextsJobHandler',
...
/**
 * Dead Letter Queue handler for failed delete-expired-martin-contexts ticks.
 *
 ...
 *
 * @param {PgBoss.Job<IDeleteExpiredMartinContextsJobData>[]} jobs - The failed jobs.
 * @return {Promise<void>}
 */
export const deleteExpiredMartinContextsFailedHandler: PgBoss.WorkHandler<IDeleteExpiredMartinContextsJobData> = async (
  jobs
) => {
  for (const job of jobs) {
    defaultLog.error({
      label: 'deleteExpiredMartinContextsFailedHandler',
