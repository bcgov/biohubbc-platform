...
describe('Tile search function (integration)', function () {
  ...
  const createContext = async (options: {
    ...
    const martinContextId = result.rows[0].martin_context_id;

    for (const featureId of options.materializedFeatureIds ?? []) {
      await connection.sql(SQL`
        INSERT INTO martin_context_feature (martin_context_id, submission_feature_id)
        VALUES (${martinContextId}, ${featureId});
      `);
    }

    return martinContextId;
  };
  ...
  const renderTile = async (martinContextId: string | null, zoom = 12): Promise<number | null> => {
    ...
    const result = await connection.sql(
      SQL`
        ...
        SELECT length(biohub.martin_search(t.z, t.x, t.y, ${params}::json)) AS tile_bytes FROM t;
      `,
