...
describe('Download version export state machine (integration)', function () {
  ...
  // ── Export listing by download version (public gallery contents) ─────
  // The gallery passes each download's displayed (latest) version id, so this returns
  // the exports of exactly those versions: a version with no live exports yields
  ...
  describe('listExportsByDownloadVersionIds', () => {
    ...
    it('returns the exports of exactly the requested version', async () => {
      ...
      const result = await exportRepo.listExportsByDownloadVersionIds([v1]);
      expect(result.map((r) => r.download_version_export_id)).to.eql([v1Export.download_version_export_id]);
    });

    it('returns an empty list for a version that has no exports — no fallback to an older version', async () => {
      const { downloadId, downloadVersionId: v1, systemUserId } = await seedReadyDownloadWithVersionArtifact();
      stubPublish();
      // V1 has an export, but the newest version (V2) is export-less (a fresh re-run before rebuild).
      ...
      // The gallery passes the latest version (V2); it has no exports, so the result is empty.
      const result = await exportRepo.listExportsByDownloadVersionIds([v2]);
      expect(result).to.eql([]);
    });

    it('scopes strictly to the given versions, never accumulating every version’s exports', async () => {
      const { downloadId, downloadVersionId: v1, systemUserId } = await seedReadyDownloadWithVersionArtifact();
      stubPublish();
      const v1Export = await exportService.createDownloadVersionExport(
        downloadId,
      ...
      // Contrast: the download-keyed batch returns BOTH versions' exports (the accumulation)...
      const byDownload = await exportRepo.listDownloadVersionExportsByDownloadIds([downloadId]);
      expect(byDownload.map((r) => r.download_version_export_id)).to.have.members([
        v1Export.download_version_export_id,
        v2Export.download_version_export_id
      ]);

      // ...whereas the version-keyed batch returns ONLY the requested version's export.
      const result = await exportRepo.listExportsByDownloadVersionIds([v2]);
      const ids = result.map((r) => r.download_version_export_id);
      expect(ids).to.eql([v2Export.download_version_export_id]);
      expect(ids).to.not.include(v1Export.download_version_export_id);
    });

    it('excludes an export whose artifact group is soft-ended', async () => {
      const { downloadId, systemUserId } = await seedReadyDownloadWithVersionArtifact();
      stubPublish();

      // V2 gets an export, but its group is then soft-ended (retired failure).
      const v2 = await addReadyVersion(downloadId);
      const v2Export = await exportService.createDownloadVersionExport(
        downloadId,
        systemUserId,
      ...
      // V2's only export group is soft-ended, so the version surfaces no live exports.
      const result = await exportRepo.listExportsByDownloadVersionIds([v2]);
      expect(result).to.eql([]);
    });
  });
