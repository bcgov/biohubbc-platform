...
export class SubmissionUploadStatusRepository extends BaseRepository {
  ...
  private _makeGetSubmissionUploadStatusQuery(submissionId: number): Knex.QueryBuilder {
      ...
      .with('scans_and_files', (qb) => {
        ...
        COALESCE(jsonb_agg(
          jsonb_build_object(
            ...
            'scanned_at', artifact_security_scan.scanned_at::text,
            'results', COALESCE(artifact_security_scan.results::jsonb, '{}'::jsonb)
    ...
    `),
        knex.raw(`
      (
        SELECT COALESCE(jsonb_object_agg(role, jsonb_build_object('count', count, 'byte_size', byte_size)), '{}')
        FROM (
          SELECT 
            role,
            COUNT(*)::int AS count,
