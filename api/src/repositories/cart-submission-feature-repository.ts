...
export class CartSubmissionFeatureRepository extends BaseRepository {
  /**
   * Add multiple submission features to a cart.
   * Ignores existing relationships (idempotent).
   * Verifies cart ownership via systemUserId.
   *
  ...
  async addSubmissionFeaturesToCart(
    ...
    const query = knex
      .with('w_cart', (qb) => {
        qb.select('cart_id').from('cart').where('cart_id', cartId).andWhere('system_user_id', systemUserId);
      })
      .with('w_features', (qb) => {
        qb.select(knex.raw('unnest(?) AS submission_feature_id', [submissionFeatureIds]));
      })
      ...
      .insert(
        knex.raw(`
        INTO cart_submission_feature (cart_id, submission_feature_id)
  ...
  /**
   * Remove submission features from a cart.
   * Verifies cart ownership via systemUserId.
   *
   * @param {string} cartId - The ID of the cart
  ...
  /**
   * Get all submission features in a cart with pagination.
  ...
  /**
   * Get the total number of submission features in a cart.
