...
describe('SubmissionFeaturePropertyTaxonRepository', () => {
  ...
  describe('insert', () => {
    ...
    it('bulk inserts rows', async () => {
      const mockDBConnection = getMockDBConnection({ knex: () => Promise.resolve(mockQueryResult([mockRow], 1)) });
      const repository = new SubmissionFeaturePropertyTaxonRepository(mockDBConnection);

      const result = await repository.insertSubmissionFeaturePropertyTaxonMany([
        {
          submission_feature_id: 10,
          feature_type_property_id: 20,
          taxon_id: 1234
    ...
    it('throws on failed bulk insert', async () => {
      const mockDBConnection = getMockDBConnection({ knex: () => Promise.resolve(mockQueryResult([], 0)) });
      const repository = new SubmissionFeaturePropertyTaxonRepository(mockDBConnection);

      try {
        await repository.insertSubmissionFeaturePropertyTaxonMany([
          {
            submission_feature_id: 10,
            feature_type_property_id: 20,
            taxon_id: 1234
