...
describe('TeamAuthorizationRepository', () => {
  ...
  describe('isUserAuthorizedForDownload', () => {
    it('returns true for an existing unclaimed download when the user is anonymous', async () => {
      ...
      const repository = new TeamAuthorizationRepository(mockConnection);
      const result = await repository.isUserAuthorizedForDownload(null, 'aaaa0000-0000-0000-0000-000000000001');

      expect(result).to.be.true;
      const sql = sqlStub.firstCall.args[0].text.toLowerCase();
      expect(sql).to.include('from download d');
      expect(sql).to.include('not exists');
      expect(sql).to.include('download_team');
    });

    it('returns true for a claimed download when the user is a linked team member', async () => {
      const sqlStub = sinon.stub().resolves({
        rowCount: 1,
        rows: [{ authorized: true }]
      } as QueryResult<any>);
      ...
      const repository = new TeamAuthorizationRepository(mockConnection);
      const result = await repository.isUserAuthorizedForDownload(42, 'aaaa0000-0000-0000-0000-000000000001');

      expect(result).to.be.true;
      const sql = sqlStub.firstCall.args[0].text.toLowerCase();
      expect(sql).to.include('join team t on t.team_id = dt.team_id');
      expect(sql).to.include('join team_member tm on tm.team_id = dt.team_id');
      expect(sql).to.include('tm.system_user_id');
      expect(sql).to.include('dt.record_end_date is null');
      expect(sql).to.include('t.record_end_date is null');
      expect(sql).to.include('tm.record_end_date is null');
    });

    it('returns false when the user is not authorized for the download', async () => {
      const mockConnection = getMockDBConnection({
      ...
      const repository = new TeamAuthorizationRepository(mockConnection);
      const result = await repository.isUserAuthorizedForDownload(99, 'aaaa0000-0000-0000-0000-000000000001');

      expect(result).to.be.false;
    });

    it('returns false when no authorization row is returned', async () => {
      const mockConnection = getMockDBConnection({
        sql: sinon.stub().resolves({
          rowCount: 0,
          rows: []
