  ...
  components: {
    securitySchemes: {
      ...
      ApiKey: {
        ...
        name: 'X-API-Key',
        description:
          "To access API-key-gated routes, a valid BioHub API key must be present in the 'X-API-Key' header. API keys are generated per user account and expire after 6 months. The key format is `biohub_<prefix>_<secret>`. This security scheme should be used on endpoints that are intended for non-interactive scripts and external systems."
      }
    },
    responses: {
      '400': {
