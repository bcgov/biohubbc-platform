 ...
 *
 * Carries only the fields the read queries project (`gallery_id`, `name`, `slug`,
 * `visibility`, `description`, `create_date`) — `record_end_date` is intentionally
 * absent since every gallery read filters to active rows, so it never appears in
 * the returned shape.
 */
