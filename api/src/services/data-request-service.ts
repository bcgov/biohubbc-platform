...
export class DataRequestService extends DBService {
  ...
  /**
   * Create the default policy used by ticket-linked data requests.
   *
   * @private
   * @return {Promise<{ policy_id: string; status: PolicyStatus }>} Created policy ID and status.
   * @memberof DataRequestService
   */
  private async createPolicy(): Promise<{ policy_id: string; status: PolicyStatus }> {
