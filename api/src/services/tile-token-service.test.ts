...
import { resetTileTokenSigningKey, TileTokenService } from './tile-token-service';
...
describe('TileTokenService', () => {
  ...
  beforeEach(() => {
    resetTileTokenSigningKey();

    process.env.TILE_TOKEN_PRIVATE_KEY_PATH = '/fake/private.pem';
    process.env.TILE_TOKEN_KID = '2026-07';
    process.env.TILE_TOKEN_AUD = 'biohub-tiles';
    process.env.TILE_TOKEN_ISS = 'biohub-api';
    process.env.TILE_TOKEN_TTL_SECONDS = '900';
  ...
  describe('mintToken', () => {
    it('signs a verifiable RS256 token carrying the expected claims', () => {
      const { token, expiresIn, jti } = new TileTokenService().mintToken({ source: 'fixture', ctx: 'ctx-123' });
    ...
    it('sets the key id header so the gateway can select a public key', () => {
      const { token } = new TileTokenService().mintToken({ source: 'fixture', ctx: 'ctx-123' });
    ...
    it('throws a 500 when the signing key path is not configured', () => {
      delete process.env.TILE_TOKEN_PRIVATE_KEY_PATH;

      try {
        new TileTokenService().mintToken({ source: 'fixture', ctx: 'ctx-1' });
    ...
    it('throws a 500 when the key id is not configured', () => {
      delete process.env.TILE_TOKEN_KID;
  ...
  describe('getTileUrlTemplate', () => {
    it('returns a relative template by default, so tiles are same origin', () => {
      delete process.env.TILE_EXTERNAL_URL;

      expect(new TileTokenService().getTileUrlTemplate('fixture')).to.equal('/tiles/fixture/{z}/{x}/{y}');
    });

    it('honours a configured base url and trims a trailing slash', () => {
      process.env.TILE_EXTERNAL_URL = 'http://localhost:6300/tiles/';

      expect(new TileTokenService().getTileUrlTemplate('fixture')).to.equal(
        'http://localhost:6300/tiles/fixture/{z}/{x}/{y}'
      );
    });
  });
});
