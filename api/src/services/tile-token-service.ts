...
const defaultLog = getLogger('services/tile-token-service');
...
export interface MintedTileToken {
  token: string;
  expiresIn: number;
  jti: string;
}
 ...
 */
export const resetTileTokenSigningKey = () => {
...
const getPrivateKey = (): string => {
  ...
  const keyPath = process.env.TILE_TOKEN_PRIVATE_KEY_PATH;
 ...
 *
 * Asymmetric (RS256) rather than symmetric signing, so the tile gateway only ever holds the public
...
export class TileTokenService {
   ...
   *
   * @param {TileTokenClaims} claims
   * @return {*}  {MintedTileToken}
   */
  mintToken(claims: TileTokenClaims): MintedTileToken {
    const privateKey = getPrivateKey();

    const keyId = process.env.TILE_TOKEN_KID;

    if (!keyId) {
      throw new HTTP500('Tile tokens are not configured');
    }

    const expiresIn = Number(process.env.TILE_TOKEN_TTL_SECONDS) || DEFAULT_TOKEN_TTL_SECONDS;
    const jti = uuidv4();

    const token = jwt.sign({ source: claims.source, ctx: claims.ctx, scope: 'tiles:read' }, privateKey, {
      algorithm: 'RS256',
      keyid: keyId,
      audience: process.env.TILE_TOKEN_AUD || 'biohub-tiles',
      issuer: process.env.TILE_TOKEN_ISS || 'biohub-api',
      jwtid: jti,
      expiresIn
    });
   ...
   *
   * Relative by default (`/tiles/...`). In OpenShift the gateway is exposed under `/tiles` on the
   * app's own hostname, so tiles are same origin; locally the app's dev server proxies `/tiles` to
   * the gateway. Keeping it relative means no environment specific host has to be threaded through.
   *
   * @param {string} source
   * @return {*}  {string}
   */
  getTileUrlTemplate(source: string): string {
    const baseUrl = process.env.TILE_EXTERNAL_URL || '/tiles';
