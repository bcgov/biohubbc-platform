...
describe('TicketService', () => {
  ...
  describe('createTicket', () => {
    ...
    it('retries when generated short ID collides on unique constraint', async () => {
      const duplicateError: any = new Error('duplicate key value violates unique constraint');
      duplicateError.code = '23505';
      duplicateError.constraint = 'ticket_short_id_unique';
  ...
  describe('getTicket', () => {
    ...
    it('returns ticket payload with inline history when resolved by short ID', async () => {
      const history = [
        {
          ticket_status_history_id: '33333333-3333-3333-3333-333333333333',
          ticket_id: mockTicket.ticket_id,
      ...
      const getByShortIdStub = sinon.stub(TicketRepository.prototype, 'getTicketByShortId').resolves(mockTicket as any);
      const getByIdStub = sinon.stub(TicketRepository.prototype, 'getTicketById').resolves(mockTicket as any);
      const getHistoryStub = sinon.stub(TicketRepository.prototype, 'getTicketStatusHistory').resolves(history as any);

      const result = await service.getTicket(mockTicket.ticket_short_id);

      expect(getByShortIdStub).to.have.been.calledWith(mockTicket.ticket_short_id);
      expect(getByIdStub).to.have.been.calledWith(mockTicket.ticket_id);
      expect(getHistoryStub).to.have.been.calledWith(mockTicket.ticket_id);
      expect(result).to.eql({ ...mockTicket, history });
    });

    it('throws HTTP400 when ticket reference format is invalid', async () => {
      try {
        await service.getTicket('bad-ref');
        expect.fail();
      } catch (error: any) {
  ...
  describe('updateTicket', () => {
    ...
    it('updates and inserts status history when status changes', async () => {
      ...
      sinon.stub(TicketRepository.prototype, 'getTicketByShortId').resolves(mockTicket as any);
      const updateStub = sinon.stub(TicketRepository.prototype, 'updateTicket').resolves(updated as any);
      const historyStub = sinon.stub(TicketRepository.prototype, 'insertTicketStatusHistory').resolves({} as any);

      const result = await service.updateTicket(mockTicket.ticket_short_id, { status: 'CLOSED' });
