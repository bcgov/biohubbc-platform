...
/**
 * Per IP rate limiter for the tile session endpoint.
 *
 ...
 */
export const tileTokenRateLimiter = rateLimit({
  windowMs: (Number(process.env.TILE_MINT_RATE_LIMIT_WINDOW_MINUTES) || DEFAULT_WINDOW_MINUTES) * 60 * 1000,
  limit: Number(process.env.TILE_MINT_RATE_LIMIT) || DEFAULT_LIMIT,
  ...
  message: { name: 'Too Many Requests', status: 429, message: 'Too many tile session requests' }
});
